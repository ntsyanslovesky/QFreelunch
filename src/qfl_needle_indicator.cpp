...
#include "qfl_needle_indicator.h"
#include <cmath>

#define M_PI 3.1415926535

QflNeedleIndicator::QflNeedleIndicator(QWidget *parent) : QWidget(parent) {
...
void QflNeedleIndicator::animate() {
  qreal p, s, f;
  animFrame++;
  ...
  s = 0;
  for (int i = 1; i <= 10; i++)
    s += 10.0 / static_cast<qreal>(i);  // Fix this crap
  f = p / s;
...
qreal QflNeedleIndicator::getValue(void) { return value; }

void QflNeedleIndicator::startAnimation(void) {
  animFrame = 0;
...
void QflNeedleIndicator::resizeEvent(QResizeEvent *event) {
  QWidget::resizeEvent(event);
}

void QflNeedleIndicator::paintEvent(QPaintEvent *event) {
  drawBackground();
  drawNeedle();
  QWidget::paintEvent(event);
}

void QflNeedleIndicator::setMajorTicks(int t) {
...
void QflNeedleIndicator::setMinorTicks(int t) {
  minorTicks = static_cast<qreal>(t);
  update();
}

void QflNeedleIndicator::setDigitFont(QFont f) {
  digitFont = f;
  update();
}

void QflNeedleIndicator::setLabelFont(QFont f) {
  labelFont = f;
  update();
}

void QflNeedleIndicator::setAnimated(bool anim) { animated = anim; }

bool QflNeedleIndicator::isAnimated(void) { return animated; }

void QflNeedleIndicator::setValue(qreal v) {
...
void QflNeedleIndicator::setLabel(QString l) {
  label = l;
  update();
}

void QflNeedleIndicator::setDigitFormat(QString format) {
  scaleFormat = format.toLatin1();
}

void QflNeedleIndicator::setRange(qreal mi, qreal ma) {
  min = mi;
  max = ma;
  step = (max - min) / (majorTicks - 1);
  update();
...
void QflNeedleIndicator::setMinValue(qreal mi) { setRange(mi, max); }

void QflNeedleIndicator::setMaxValue(qreal ma) { setRange(min, ma); }

void QflNeedleIndicator::setGapAngle(qreal gap) {
  gap_angle = gap;
  start_angle = 90 + gap_angle / 2;
  stop_angle = 360 - gap_angle;
  rot_deg = stop_angle / (majorTicks - 1);
...
void QflNeedleIndicator::setLabelOffset(qreal offset) {
...
qreal QflNeedleIndicator::value2angle(qreal val) {
...
void QflNeedleIndicator::drawNeedle(void) {
  int side = qMin(width(), height());

  static const QPoint needle[5] = {QPoint(0, -2), QPoint(100, 0), QPoint(0, 2),
                                   QPoint(-30, 5), QPoint(-30, -5)};
...
void QflNeedleIndicator::drawBackground(void) {
  int side = qMin(width(), height());
  ...
  for (int i = 0; i < majorTicks; i++) {
    QPointF point((70 * cos(rotation)),
                  70 * sin(rotation)); /* calculate digit coords      */
    QString value = QString().sprintf(
        scaleFormat.constData(),
        min + (qreal)i * step); /* convert digit to string     */
    QSize size = painter.fontMetrics().size(
        Qt::TextSingleLine, value);  /* get string size in px       */
  ...
  if (label.size()) {
    ...
    point.rx() -= size.width() / 2; /* center-align string (horiz) */
    point.ry() +=
        size.height() / 4 + labelOffset; /* shift down, below needle    */

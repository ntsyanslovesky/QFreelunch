...
class QflNeedleIndicator : public QWidget {
  Q_OBJECT
 ...
 public:
  /**
   * Creates QflNeedleIndicator widget with default settings.
   */
  QflNeedleIndicator(QWidget *parent = 0);
   ...
   * and shield edge. 0 is needle level, 1 is edge level, 0.5 is between needle
   * and edge, 0.75 is closer to the edge, etc. Value must be between 0 and 1.
  ...
  /**
   * Get currently set value. This is NOT actual needle position during
   * animation, but final indicator value.
 ...
 private:
  ...
  qreal start_angle, stop_angle,
      gap_angle;  // scale angles used for step/rotations/etc calculation
  QFont digitFont;
  QFont labelFont;
  QString label;
  ...
  qreal valueDelta;   // Delta between current and target needle position value.
                      // Used in animation
  bool animated;      // Animation flag. Animate needle when true
  qreal labelOffset;  // Label position between axis needle (when 0.0) and
                      // shield edge (when 1.0)
